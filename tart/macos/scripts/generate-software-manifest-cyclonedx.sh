...
cat > "${TEMP_FILE}" << EOF
{
  ...
  "metadata": {
    ...
    "component": {
      ...
      "name": "${BOM_NAME}",
      "version": "${BOM_VERSION}",
      "description": "MAUI development environment with .NET, Android SDK, Xcode, and development tools"
    }
  },
  "components": [
EOF
...
# Add macOS as operating-system component
MACOS_DESC="macOS ${OS_VERSION} build ${OS_BUILD}, architecture ${ARCHITECTURE}"
add_component "pkg:generic/macos@${OS_VERSION}" "operating-system" "macOS" "${OS_VERSION}" "Apple Inc." "${MACOS_DESC}" "${FIRST_COMPONENT}"
...
if jq -e '.xcode.defaultVersion' "${SOURCE_JSON}" >/dev/null 2>&1; then
  ...
  if [[ "${XCODE_VERSION}" != "null" && "${XCODE_VERSION}" != "unknown" ]]; then
    XCODE_DESC="Xcode IDE and development tools"
    [[ -n "${XCODE_BUILD}" && "${XCODE_BUILD}" != "null" ]] && XCODE_DESC="${XCODE_DESC} (Build ${XCODE_BUILD})"
    add_component "pkg:generic/xcode@${XCODE_VERSION}" "application" "Xcode" "${XCODE_VERSION}" "Apple Inc." "${XCODE_DESC}" "${FIRST_COMPONENT}"
    FIRST_COMPONENT=false
  fi
fi
...
echo "        \"pkg:generic/macos@${OS_VERSION}\"" >> "${TEMP_FILE}"
echo "      ]" >> "${TEMP_FILE}"
echo "    }" >> "${TEMP_FILE}"
echo "  ]" >> "${TEMP_FILE}"
echo "}" >> "${TEMP_FILE}"
